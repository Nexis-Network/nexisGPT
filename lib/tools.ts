...
export const TOOL_SCHEMAS: Record<ToolName, FunctionTool> = {
  ...
  [ToolName.CreateAgent]: {
    ...
    function: {
      name: "createAgent",
      description: `Create and publish an AI agent (assistant) to the NexisGPT Agents repository.  Agents are generally for Solidity smart contract development but can also be created for anything else.  All agents have these tools available: ${DEFAULT_TOOLS.map(
        (tool) => tool
      ).join(", ")}`,
      parameters: {
        type: "object",
